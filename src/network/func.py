...
def get_vertices(input_dir):
    point_unique_list = []
    road_unique_list = []
    geopoint_list = []
    road_length_list = []
    start_id = 0
    with fiona.open(f'{input_dir}/target_road.gpkg') as lines:
        for line in lines:
            # raod start point
...
def select_rep_point(df, inter_list):
    rep_point_list = []
    for i in inter_list:
...
# get nearest points
def get_neareset_points(df, start_df, goal_df):
